...
// a channel for communicating when the generation has ended.
//
...
// CommitInterval.  Otherwise, they must be managed by the client.
type Generation struct {
	...
	// down any Readers that have been opened.
	Done <-chan struct{}
...
// if a higher offset has not been previously marked.  Use SetOffset to prepare
// a specific offset.
func (g *Generation) MarkOffset(topic string, partition int, offset int64) {
	g.offsetLock.Lock()
	partOffsets, ok := g.offsets[topic]
	if !ok {
		partOffsets = make(map[int]int64)
...
// SetOffset prepares the offset for the topic+partition for commit regardless
// of any previously prepared offsets.
func (g *Generation) SetOffset(topic string, partition int, offset int64) {
	g.offsetLock.Lock()
	partOffsets, ok := g.offsets[topic]
	if !ok {
		partOffsets = make(map[int]int64)
...
// CommitOffsets commits all offsets that have been prepared with either
// MarkOffset or SetOffset.
func (g *Generation) CommitOffsets() error {
	...
	// on success, remove any unchanged partitions from the map.  this will keep
	// us from continually committing offsets e.g. when the consumer is stalled.
	// this is the only place where data is removed from the maps, so we don't
	// need to check existence prior to deletion or access.
	if err == nil {
		g.offsetLock.Lock()
		for _, t := range request.Topics {
			for _, p := range t.Partitions {
				if p.Offset == g.offsets[t.Topic][int(p.Partition)] {
					delete(g.offsets[t.Topic], int(p.Partition))
				}
			}
			if len(g.offsets[t.Topic]) == 0 {
				delete(g.offsets, t.Topic)
			}
		}
		g.offsetLock.Unlock()
...
func (g *Generation) commitLoop(interval time.Duration) {
	...
	commit := func() {
		if err := g.CommitOffsets(); err != nil {
	...
	for {
		select {
		case <-g.Done:
			// commit final state when ending this generation.
			commit()
			return
		case <-ticker.C:
			commit()
...
func (cg *ConsumerGroup) Next(ctx context.Context) (next Generation, err error) {
	select {
	case <-ctx.Done():
		return next, ctx.Err()
	case <-cg.done:
		return next, ErrGroupClosed
	case err = <-cg.errs:
		return next, err
	case next = <-cg.next:
		return next, nil
