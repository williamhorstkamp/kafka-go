...
func (r *Reader) commitOffsetsWithRetry(current Generation, retries int) (err error) {
	const (
	...
	for attempt := 0; attempt < retries; attempt++ {
		...
		if err = current.CommitOffsets(); err == nil {
			return
		}
	}
...
func (r *Reader) commitLoopImmediate(current Generation) {
	for {
		select {
		case <-current.Done:
			return
		...
		case req := <-r.commits:
			for _, c := range req.commits {
				current.MarkOffset(c.topic, c.partition, c.offset)
			}
			req.errch <- r.commitOffsetsWithRetry(current, defaultCommitRetries)
		}
	}
}
...
func (r *Reader) commitLoopInterval(current Generation) {
	ticker := time.NewTicker(r.config.CommitInterval)
	defer ticker.Stop()

	commit := func() {
		if err := r.commitOffsetsWithRetry(current, defaultCommitRetries); err != nil {
...
func (r *Reader) commitLoop(current Generation) {
	r.withLogger(func(l *log.Logger) {
	...
	if r.config.CommitInterval == 0 {
		r.commitLoopImmediate(current)
	} else {
		r.commitLoopInterval(current)
	}
}
